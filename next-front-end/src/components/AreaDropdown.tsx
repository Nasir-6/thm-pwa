...
import MosqueNameLink from './MosqueNameLink';
...
const AreaDropdown = ({ area, mosques }: Props) => {
  const mosqueLinks = mosques.map((mosque) => (
    <li className="hover:text-accent-600 flex gap-1">
      <MosqueNameLink mosqueName={mosque.name} mosqueUrlSlug={mosque.urlSlug} key={mosque.name} />
      {mosque.hasFemaleFacilities ? <FemaleIcon /> : null}
      {mosque.hasWheelchairAccess ? <WheelchairIcon /> : null}
    </li>
  ));
  return (
    <div className="area-container px-3 pb-5 min-w-[330px]">
      <label
      ...
      <div className="wrapper mt-2">
        <ul className="pl-9 text-primary-500 max-h-[134px] overflow-scroll">{mosqueLinks}</ul>
      </div>
    </div>
  );
};
